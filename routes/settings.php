...
$r->group( '/wp-admin', function ( RouteCollectionInterface $r ) {
	...
	$r->post( '/admin.php?page=dt-autolink&tab=general', [ GeneralSettingsController::class, 'update' ] )->middleware( new Nonce( config( 'plugin.dt_admin_form_nonce' ) ) );
	$r->post( '/admin.php?page=dt-autolink', [ GeneralSettingsController::class, 'update' ] )->middleware( new Nonce( config( 'plugin.dt_admin_form_nonce' ) ) );
