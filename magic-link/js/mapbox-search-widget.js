...
function write_results_box() {
  ...
  if (
    (dtMapbox.post.location_grid_meta !== undefined &&
      dtMapbox.post.location_grid_meta.length !== 0) ||
    (dtMapbox.post.contact_address !== undefined &&
      dtMapbox.post.contact_address.length !== 0)
  ) {
    if (
      dtMapbox.post.location_grid_meta !== undefined &&
      dtMapbox.post.location_grid_meta.length !== 0
      ...
      jQuery.each(dtMapbox.post.location_grid_meta, function (i, v) {
        if (v.grid_meta_id) {
          lgm_results.append(`<div class="input-group">
            <input type="text" class="active-location input-group-field" id="location-${window.lodash.escape(
              v.grid_meta_id
          ...
          lgm_results.append(`<div class="input-group">
            <input type="text" class="dt-communication-channel input-group-field" id="${window.lodash.escape(
              v.key
            )}" value="${window.lodash.escape(
            v.label
          )}" dir="auto" data-field="contact_address" />
            <div class="input-group-button">
              ...
              <button type="button" class="button alert input-height delete-button-style channel-delete-button delete-button" title="${
                window.lodash.escape(
                  dtMapbox.translations.delete_location
                ) /*Delete Location*/
              }" data-id="${window.lodash.escape(
            v.key
          )}" data-field="contact_address" data-key="${window.lodash.escape(
            v.key
          )}">&times;</button>
            </div>
          </div>`);
...
function delete_click_listener() {
  jQuery(".mapbox-delete-button").on("click", function () {
    ...
    API.update_post(dtMapbox.post_type, dtMapbox.post_id, data)
...
function load_modal(lng, lat, level, label, grid_id) {
  ...
  content.empty().append(`
           <div class="grid-x">
            <div class="cell"><strong>${window.lodash.escape(
              label
            )}</strong></div>
            <div class="cell">
                <div id="map-wrapper">
...
window.write_input_widget = function write_input_widget() {
  if (jQuery("#mapbox-autocomplete").length === 0) {
    ...
    <div id="mapbox-autocomplete" class="mapbox-autocomplete input-group" data-autosubmit="true" data-add-address="true">
        <input id="mapbox-search" type="text" name="mapbox_search" class="input-group-field" autocomplete="off" dir="auto" placeholder="${
          dtMapbox.translations.search_location /*Search Location*/
        }" />
        <div class="input-group-button">
            ...
            <button id="mapbox-clear-autocomplete" class="button alert input-height delete-button-style mapbox-delete-button" type="button" title="${
              window.lodash.escape(
                dtMapbox.translations.clear
              ) /*Delete Location*/
  ...
  mapbox_search.on("keyup", function (e) {
    ...
    if (e.which === 40) {
      ...
      if (window.currentfocus > -1) {
        ...
        close_all_lists(
          $($("#mapbox-autocomplete-list div")[window.currentfocus]).data(
            "value"
          )
        );
      }
  ...
  fetch(url, {
    ...
    .then((data) => {
      ...
      jQuery.each(data.features, function (index, value) {
        if (4 > index) {
          list.append(
            `<div data-value="${window.lodash.escape(
              index
            )}">${window.lodash.escape(value.place_name)}</div>`
          );
        }
      });
      ...
      if (typeof add_address === "undefined" || add_address === true) {
        list.append(
          `<div data-value="address" style="font-weight:bold;">${window.lodash.escape(
            window.dtMapbox.translations.use
          )}: <span dir="auto">"${window.lodash.escape(address)}"</span></div>`
        );
      }
    ...
    function (predictions, status) {
      ...
      if (status === "OK") {
        jQuery.each(predictions, function (index, value) {
          if (4 > index) {
            list.append(
              `<div data-value="${index}">${window.lodash.escape(
                value.description
              )}</div>`
            );
          }
        ...
        if (typeof add_address === "undefined" || add_address === true) {
          list.append(
            `<div data-value="address" style="font-weight:bold;">${window.lodash.escape(
              window.dtMapbox.translations.use
            )}: <span dir="auto">"${window.lodash.escape(
              address
            )}"</span></div>`
          );
        }
...
function post_geocoded_location() {
  if (jQuery("#mapbox-autocomplete").data("autosubmit")) {
    ...
    API.update_post(dtMapbox.post_type, dtMapbox.post_id, window.location_data)
      .then(function (response) {
        dtMapbox.post = response;
...
function post_contact_address(update) {
  ...
  if (mapbox_autocomplete.data("autosubmit")) {
    ...
    API.update_post(window.dtMapbox.post_type, window.dtMapbox.post_id, {
